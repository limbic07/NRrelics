...
class RepoCleaner:
    ...
    def start_cleaning(self, mode: str, cleaning_mode: str, max_relics: int,
        ...
        try:
            # 1. 应用筛选
            log("正在应用遗物筛选...", "INFO")
            self.repository_filter.apply_filter(mode)
            time.sleep(1.0)
            ...
            while self.is_running:
                ...
                log(f"[{self.stats['total_detected']}] 遗物状态: {relic_state}", "INFO")
                ...
                if match_result["qualified"]:
                    ...
                    log(f"不合格遗物 ({match_result['reason']})", "WARNING")
                    self.stats["unqualified"] += 1

                # 操作执行
                self._execute_action(relic_state, match_result["qualified"], cleaning_mode, log)
    ...
    def _execute_action(self, relic_state: str, is_qualified: bool, cleaning_mode: str, log):
        """执行操作"""
        if cleaning_mode == "sell":
            if is_qualified:
                # 合格遗物跳过
    ...
    def _print_stats(self, log):
        ...
        log(f"不合格: {self.stats['unqualified']}", "WARNING")
